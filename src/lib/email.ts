...
export async function sendContractSigningEmail(params: {
  ...
  clientName: string;
  contractUrl: string;
  /** e.g. "Mutual Non-Disclosure Agreement"; defaults to the service agreement wording */
  documentTitle?: string;
}) {
  const docName = params.documentTitle || "service agreement";
  const subjectName = params.documentTitle || "Agreement";
  const resend = getResend();
  if (!resend) {
    console.warn("[Email] RESEND_API_KEY not set, skipping email");
    return null;
  ...
  const { data, error } = await resend.emails.send({
    from: FROM_EMAIL,
    to: params.to,
    subject: `Please Review & Sign Your ${subjectName} | Blok Blok Studio`,
    html: `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 20px;">
        <div style="text-align: center; margin-bottom: 32px;">
          <h2 style="color: #111; margin: 0;">Blok Blok Studio</h2>
        ...
        <p style="color: #333; font-size: 16px; line-height: 1.6;">
          Your ${docName} is ready for review and signature. Please click the button below to review the terms and sign electronically.
        </p>
        <div style="text-align: center; margin: 32px 0;">
...
export async function sendClientReportEmail(params: {
  ...
  highlights: string[];
  summary: string[];
  trajectory: Array<{ label: string; points: Array<{ month: string; value: string }>; direction: "up" | "down" | "flat"; note: string }>;
  ...
  for (let i = 0; i < cells; i += 2) {
    ...
    metricRows.push(
      `<tr>${slice
        .map(
          (m) => `
        <td width="50%" style="padding: 12px 16px; border: 1px solid #eee; border-left: 4px solid ${!m.change ? "#e5e7eb" : m.change.trim().startsWith("-") ? "#dc2626" : m.change.trim().startsWith("+") ? "#059669" : "#d97706"}; text-align: center;">
          <div style="font-size: 22px; font-weight: 700; color: #111;">${esc(m.value)}</div>
          <div style="font-size: 12px; color: #666; margin-top: 2px;">${esc(m.label)}</div>
          ${m.change ? `<div style="font-size: 12px; font-weight: 600; margin-top: 2px; color: ${m.change.trim().startsWith("-") ? "#dc2626" : "#059669"};">${esc(m.change)}</div>` : ""}
        </td>`
  ...
  const { data, error } = await resend.emails.send({
    ...
    html: `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 620px; margin: 0 auto; background: #ffffff;">
        <!-- Brand header: black block, logo, orange rule -->
        <div style="background: #0A0A0A; padding: 28px 24px; text-align: center; border-bottom: 4px solid #FF6B00;">
          <img src="https://blokblokstudio-clients.vercel.app/bb_logo_wordmark_subhead_WHT_PNG.png" alt="Blok Blok Studio" width="180" style="max-width: 180px; height: auto;" />
        </div>

        <div style="padding: 32px 24px;">
          <h1 style="color: #111; font-size: 22px; margin: 0 0 4px;">${esc(params.monthLabel)} Performance Report</h1>
          <p style="color: #666; font-size: 14px; margin: 0 0 24px;">Prepared for ${esc(params.clientName)}${params.preparedBy ? ` by ${esc(params.preparedBy)}` : ""} · Blok Blok Studio</p>

          <table width="100%" cellspacing="0" cellpadding="0" style="border-collapse: collapse; margin-bottom: 28px;">
            ${metricRows.join("")}
          </table>

          ${params.trajectory.length ? `
          <h3 style="color: #111; font-size: 16px; margin: 0 0 12px;">Growth trajectory</h3>
          ${params.trajectory
            .map((t) => {
              const color = t.direction === "up" ? "#059669" : t.direction === "down" ? "#dc2626" : "#d97706";
              const arrow = t.direction === "up" ? "▲" : t.direction === "down" ? "▼" : "▶";
              const chips = t.points
                .map(
                  (pt, i) => `<span style="display: inline-block; background: ${i === t.points.length - 1 ? color : "#f3f4f6"}; color: ${i === t.points.length - 1 ? "#fff" : "#374151"}; border-radius: 6px; padding: 4px 10px; font-size: 12px; margin: 0 6px 6px 0;"><span style="opacity: 0.75;">${esc(pt.month.slice(5))}/${esc(pt.month.slice(2, 4))}</span>&nbsp; <strong>${esc(pt.value)}</strong></span>`
                )
                .join(`<span style="color: #bbb; font-size: 11px;">→ </span>`);
              return `
              <div style="border-left: 4px solid ${color}; background: #fafafa; border-radius: 0 8px 8px 0; padding: 12px 14px; margin: 0 0 12px;">
                <p style="margin: 0 0 8px; font-size: 14px; color: #111;"><strong>${esc(t.label)}</strong> <span style="color: ${color}; font-weight: 700;">${arrow}</span></p>
                <div style="margin: 0 0 6px;">${chips}</div>
                <p style="margin: 0; font-size: 13px; color: #555; line-height: 1.5;">${esc(t.note)}</p>
              </div>`;
            })
            .join("")}
          ` : ""}

          ${params.highlights.length ? `<h3 style="color: #111; font-size: 16px; margin: 24px 0 10px;">Highlights</h3><ul style="padding-left: 20px; margin: 0 0 24px;">${list(params.highlights)}</ul>` : ""}
          ...
          <p style="color: #999; font-size: 12px; text-align: center;">
            ${params.preparedBy ? `Prepared by ${esc(params.preparedBy)} · ` : ""}Blok Blok Studio [creative tech studio]<br/>
            Questions about this report? Just reply to this email.
          </p>
        </div>
      </div>
    `,
