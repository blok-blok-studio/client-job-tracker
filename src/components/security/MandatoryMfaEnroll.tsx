...
import { ShieldCheck, Smartphone, AlertTriangle, Copy, Check, Loader2 } from "lucide-react";
...
export default function MandatoryMfaEnroll({ email }: { email: string }) {
  ...
  async function startSetup(e: React.FormEvent) {
    ...
    try {
      ...
      setQr({ qrDataUrl: json.data.qrDataUrl, secret: json.data.secret });
      setStep("scan");
    } finally {
      setBusy(false);
    }
        ...
        <div className="bg-bb-surface border border-bb-border rounded-lg p-5">
          ...
          {step === "scan" && qr && (
            <form onSubmit={confirmEnable} className="space-y-3">
              ...
              <details className="text-[11px] text-bb-dim">
                <summary className="cursor-pointer">Can&apos;t scan? Enter this key manually</summary>
                <code className="block break-all text-white bg-bb-black border border-bb-border rounded px-2 py-1 mt-1">
                  {qr.secret}
                </code>
              </details>
              <input
